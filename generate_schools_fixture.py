...
This version processes the content directly
"""
import json
import re
...
def extract_pin_code(text):
    ...
    # Look for patterns like - 600013 or 600013 or Chennai - 600013
    match = re.search(r'[-]?\s*(\d{6})', text)
    if match:
        return match.group(1)
    return "600001"  # Default Chennai pin code

def parse_school_content(content, board):
    """Parse school content from text"""
    schools = []
    ...
    while i < len(lines):
        ...
        field_labels = ['Address', 'State', 'District', 'Phone', 'Email', 'Website', ':']
        if line and line not in field_labels:
            # Start new school
            school_name = line
            i += 1
            ...
            while i < len(lines):
                ...
                # Check if we've hit the next school
                if line and line not in field_labels:
                    break
...
def generate_facilities(board, name):
    ...
    if random.random() > 0.3:
        facilities.append(1)  # AC
    
    if random.random() > 0.2:
        facilities.append(6)  # Science Lab
    ...
    for school in schools:
        ...
        entry = {
            ...
            "fields": {
                ...
                "bus_availability": random.random() > 0.3,
...
def main():
    ...
    files_to_process = [
        ('Schools_info/cbse.md', 'CBSE'),
        ('Schools_info/Ib.md', 'IB'),
        ('Schools_info/icse.md', 'ICSE')
    ]
    
    for filepath, board in files_to_process:
        try:
            ...
            print(f"Parsing {board} schools from {filepath}...")
            schools = parse_school_content(content, board)
            print(f"Found {len(schools)} {board} schools")
    ...
    if not all_schools:
        print("No schools found! Please ensure the markdown files are saved and contain data.")
        return
